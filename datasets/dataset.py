...
class KlevrDataset(Dataset):
    ...
    def read_meta(self):
        ...
        if self.split == 'train':
            ...
            for image_id in self.split_ids:
                if self.get_rgb:
                    image_path = os.path.join(self.root_dir, f'{image_id:05d}.png') #{image_id:05d}
    ...
    def __getitem__(self, idx):
        ...
        sample['fname'] = self.split_ids[idx]

        return sample

# if __name__ == "__main__":
    # dataset = KlevrDataset(root_dir='', split='val', get_rgb=True)
    # print(len(dataset))
